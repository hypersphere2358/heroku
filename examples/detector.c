...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    // baeg 추가
    char characters_pool[62] = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
    char prediction_file_name[33] = "predictions_00000000000000000000\0";
    // baeg 종료
    ...
    while(1){
        ...
        if(outfile){
            ...
            for (int i = 0; i < 20; i++)
            {
                prediction_file_name[12 + i] = characters_pool[(int)(rand() % 62)];
            }
            printf("%s\n", prediction_file_name);
            save_image(im, prediction_file_name);
            // baeg 종료
